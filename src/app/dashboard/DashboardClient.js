...
export default function DashboardClient() {
  ...
  // 📈 Legacy simulated sensor changes – disabled now because we use real core data
  /*
  useEffect(() => {
  ...
  */
  ...
  // 🌡️💡🎤 Fetch real sensor data from the core API (AHT10 + light + optional sound)
  useEffect(() => {
    if (!API_BASE_URL) {
    ...
    const fetchSensorsFromCore = async () => {
      try {
        ...
        // Temperature from core (AHT10) – clamp to a realistic range
        if (typeof data.temperature === "number" && !Number.isNaN(data.temperature)) {
          const t = Math.max(5, Math.min(40, data.temperature));
          setTemperature(t);
        }

        // Humidity from core (AHT10) – clamp to 0–100%
        if (typeof data.humidity === "number" && !Number.isNaN(data.humidity)) {
        ...
        if (data.sound && typeof data.sound.levelDb === "number" && !Number.isNaN(data.sound.levelDb)) {
